...
The historical files under `legacy/` are kept for reference and may not receive the same level of active maintenance.
...
Use `.env.local.example` or `.env.example` for placeholders and keep real credentials in `.env.local` or deployment secrets.
